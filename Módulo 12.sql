...
SELECT P.ProductKey,P.ProductName,P.UnitPrice,
CASE WHEN P.ClassName= 'Economy' THEN (1 - 0.05) * P.UnitPrice ELSE 
CASE WHEN P.ClassName= 'Regular' THEN (1 -0.07) * P.UnitPrice ELSE (1 - 0.09) * P.UnitPrice END END AS '% Desconto'
FROM DimProduct AS P

/*b) Faça uma adaptação no código para que os % de desconto de 5%, 7% e 9% sejam facilmente 
modificados (dica: utilize variáveis).*/
DECLARE @ECONOMY FLOAT =(1 - 0.05),
		@REGULAR FLOAT=(1-0.07),
		@DELUXE FLOAT=(1-0.09); 

SELECT P.ProductKey,P.ProductName,P.UnitPrice,
CASE WHEN P.ClassName= 'Economy' THEN @ECONOMY * P.UnitPrice ELSE 
CASE WHEN P.ClassName= 'Regular' THEN @REGULAR * P.UnitPrice ELSE @DELUXE * P.UnitPrice END END AS '% Desconto'
FROM DimProduct AS P
...
SELECT P.BrandName, COUNT(*),
CASE WHEN COUNT(*) >= 500 THEN 'CATEGORIA A' ELSE
CASE WHEN COUNT(*)>=100 THEN 'CATEGORIA B' ELSE 'CATEGORIA C' END END
FROM DimProduct AS P
...
CASE WHEN EmployeeCount >= 50 THEN 'Acima de 50 funcionários'
 ...
 WHEN EmployeeCount >= 10 THEN 'Entre 10 e 20 funcionários'ELSE 'Abaixo de 10 funcionários' END 
FROM DimStore
...
SELECT TOP(100) PS.ProductSubcategoryName,ROUND(AVG(P.Weight)*100,2) AS 'KG',
IIF(AVG(P.Weight)*100>=1000.0,'ROTA 2 ', 'ROTA 1') AS 'ROTA' FROM DimProduct AS P
JOIN DimProductSubcategory AS PS ON PS.ProductSubcategoryKey=P.ProductSubcategoryKey
GROUP BY PS.ProductSubcategoryName
...
SELECT S.StoreName, DATEDIFF(DAY, S.OpenDate,CASE WHEN CloseDate IS NULL THEN CURRENT_TIMESTAMP ELSE CloseDate END) AS 'Dias'
FROM DimStore AS S
